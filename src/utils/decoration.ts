 ...
 * 构建 hover 消息
 */
export const buildHoverMessage = (blameInfo: GitBlameInfo): vscode.MarkdownString => {
  ...
  hoverMessage.appendMarkdown(
    `[$(git-commit)](command:git-hints.showCommitDetails?${encodeURIComponent(JSON.stringify(blameInfo.commitId))} "show commit detail")`
  );
  hoverMessage.appendMarkdown(
    `[\`${blameInfo.commitId}\`](command:git-hints.copyCommitId?${encodeURIComponent(JSON.stringify(blameInfo.commitId))} "copy commit id")\n\n`
  ); // 换行
 ...
 * 创建装饰器选项
 */
export const buildDecorationOption = (
  line: number,
  lineText: string,
 ...
 * 为单行或所有行创建装饰器
 */
export const buildLineDecorations = (
